...
@Singleton
public class IssueExplorerRevealer {
	...
	@Inject
	public IssueExplorerRevealer(IssueExplorerView issueExplorer, EventBus eventBus, PromiseProvider promises) {
		this.tree = issueExplorer.getTree();
			...
			public void onRevealResource(final RevealResourceEvent event) {
				queue.thenPromise(new Function<Void, Promise<Void>>() {
					...
					public Promise<Void> apply(Void ignored) throws FunctionException {
						return reveal(event.getLocation(), event.isSelectionRequired(), event.isFocusRequired())
	...
	/**
	 * Search node in the issue explorer tree by storable path.
	 *
	 * @param path
	 *            path to node
	 ...
	 * @param select
	 *            select node after reveal
	 * @param isFocusRequired
	 *            whether tree should take focus after reveal
	 * @return promise object with found node or promise error if node wasn't found
	 */
	public Promise<Node> reveal(final Path path, final boolean select, final boolean isFocusRequired) {
	...
	protected void reveal(final Path path, final boolean select, final boolean isFocusRequired,
		...
		Scheduler.get().scheduleFixedDelay(new Scheduler.RepeatingCommand() {
			...
			public boolean execute() {
				...
				expandToPath(root, path, select, isFocusRequired).then(new Operation<ResourceNode>() {
					@Override
	...
	protected void expand(final ResourceNode parent, final Path segment, final boolean select,
			final boolean isFocusRequired, final AsyncCallback<ResourceNode> callback) {
