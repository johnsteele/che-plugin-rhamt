...
import org.jboss.tools.rhamt.ui.issues.explorer.IssueExplorerView.ActionDelegate;
...
@Singleton
@DynaObject
public class IssueExplorerPresenter extends BasePresenter implements ActionDelegate, ResourceChangedHandler,
		MarkerChangedHandler, SyntheticNodeUpdateEvent.SyntheticNodeUpdateHandler {

	private static final int PART_SIZE = 500;
	private final IssueExplorerView view;
	...
	@Inject
	public IssueExplorerPresenter(IssueExplorerView view, EventBus eventBus, RhamtLocalizationConstant locale,
			RhamtResources resources, ResourceNode.NodeFactory nodeFactory, SettingsProvider settingsProvider,
			AppContext appContext, RequestTransmitter requestTransmitter, NotificationManager notificationManager,
			DtoFactory dtoFactory, WorkspaceAgent workspaceAgent) {
		this.view = view;
		...
		treeExpander = new IssueExplorerTreeExpander(view.getTree(), appContext);

		registerNative();

		eventBus.addHandler(ExtensionsInitializedEvent.getType(), event -> {
			if (partStack != null) {
				// partStack.setActivePart(IssueExplorerPresenter.this);
			}
		});
	...
	public void initFileWatchers() {
		final String method = "track/issues-tree";
		...
		getTree().addExpandHandler(event -> {
			...
			if (node instanceof ResourceNode) {
				...
				requestTransmitter.newRequest().endpointId(WS_AGENT_JSON_RPC_ENDPOINT_ID).methodName(method)
						.paramsAsDto(dtoFactory.createDto(ProjectTreeTrackingOperationDto.class)
								.withPath(data.getLocation().toString()).withType(START))
						.sendAndSkipResult();
			}
		});
		...
		getTree().addCollapseHandler(event -> {
			...
			if (node instanceof ResourceNode) {
				...
				requestTransmitter.newRequest().endpointId(WS_AGENT_JSON_RPC_ENDPOINT_ID).methodName(method)
						.paramsAsDto(dtoFactory.createDto(ProjectTreeTrackingOperationDto.class)
								.withPath(data.getLocation().toString()).withType(STOP))
						.sendAndSkipResult();
			}
		});
	}
  ...
  private native void registerNative() /*-{
        ...
        IssueExplorer.expandAll = $entry(function () {
            that.@org.jboss.tools.rhamt.ui.issues.explorer.IssueExplorerPresenter::doExpand()();
        });

        IssueExplorer.collapseAll = $entry(function () {
            that.@org.jboss.tools.rhamt.ui.issues.explorer.IssueExplorerPresenter::doCollapse()();
        });

        IssueExplorer.reveal = $entry(function (path) {
            that.@org.jboss.tools.rhamt.ui.issues.explorer.IssueExplorerPresenter::doReveal(*)(path);
        });

        IssueExplorer.refresh = $entry(function () {
            that.@org.jboss.tools.rhamt.ui.issues.explorer.IssueExplorerPresenter::doRefresh()();
        });

        $wnd.IDE.IssueExplorer = IssueExplorer;
    }-*/;
			...
			if (delta.getKind() == ADDED) {
				...
				if (node != null) {
					...
					if (resource.isProject()) {
						notificationManager.notify(locale.projectRemoved(node.getName()), SUCCESS, NOT_EMERGE_MODE);
					}
				}
			} else if (delta.getKind() == UPDATED) {
				for (Node node : tree.getNodeStorage().getAll()) {
					if (node instanceof ResourceNode && ((ResourceNode) node).getData().getLocation()
